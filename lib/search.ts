...
export async function searchRecipes(filters: FilterState): Promise<RecipeSummary[]> {
  ...
  if (filters.favoritesOnly) { sql += ' AND r.is_favorite = 1'; }
  ...
  if (filters.sourceType)    { sql += ' AND r.source_type = ?'; params.push(filters.sourceType); }
  ...
  const sortMap: Record<SortOrder, string> = {
    ...
    fastest:   'ORDER BY CASE WHEN r.cook_time_min IS NULL THEN 1 ELSE 0 END, r.cook_time_min ASC',
