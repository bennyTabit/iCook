...
export default function ProfileScreen() {
  ...
  const ROWS = [
    { icon: '🌐', label: t('language'),    value: isHe ? 'עברית' : 'English',             onPress: toggleLanguage },
    { icon: '☀️', label: t('theme'),       value: isHe ? 'בהיר' : 'Light',                onPress: () => {} },
    { icon: '☁️', label: t('backup'),      value: isHe ? 'לא מסונכרן' : 'Not synced',    onPress: () => {} },
    { icon: '📷', label: t('permissions'), value: isHe ? 'מצלמה, גלריה' : 'Camera, Gallery', onPress: () => {} },
    { icon: '🥗', label: t('dietary'),     value: isHe ? 'ללא הגבלה' : 'No restriction',  onPress: () => {} },
  ];
    ...
    <SafeAreaView style={s.container}>
      ...
      <View style={s.userCard}>
        <View style={s.avatar}>
          <Text style={s.avatarText}>
            {user ? user.displayName?.[0]?.toUpperCase() ?? '?' : (isHe ? 'א' : 'A')}
          </Text>
        </View>
        <View style={{ flex: 1 }}>
      ...
      <View style={s.section}>
        {ROWS.map((row, i) => (
          <TouchableOpacity
            key={row.label}
            style={[s.row, i < ROWS.length - 1 && s.rowBorder, { flexDirection: isHe ? 'row-reverse' : 'row' }]}
            onPress={row.onPress}
            activeOpacity={0.7}
          >
            <Text style={{ fontSize: 18, width: 28 }}>{row.icon}</Text>
            <Text style={[s.rowLabel, { flex: 1, textAlign: isHe ? 'right' : 'left' }]}>{row.label}</Text>
      ...
      <View style={s.section}>
        {user ? (
          ...
          <View style={s.signInCard}>
            <Text style={[s.signInTitle, { textAlign: isHe ? 'right' : 'left' }]}>{t('signIn')}</Text>
            <Text style={[s.signInSub, { textAlign: isHe ? 'right' : 'left' }]}>{t('signInSub')}</Text>
            {[
...
const s = StyleSheet.create({
  ...
  userCard:    { flexDirection: 'row', alignItems: 'center', gap: 14, margin: 16, padding: 16, backgroundColor: Colors.surface, borderRadius: 14, borderWidth: 0.5, borderColor: Colors.border },
  ...
  section:     { marginHorizontal: 16, marginBottom: 16, backgroundColor: Colors.surface, borderRadius: 14, borderWidth: 0.5, borderColor: Colors.border, overflow: 'hidden' },
  ...
  authBtn:     { backgroundColor: Colors.background, borderRadius: 10, padding: 12, alignItems: 'center', borderWidth: 0.5, borderColor: Colors.border },
